...
resource "aws_security_group" "myapp-sg" {  # Creating a new security group
  name = "myapp-sg"
  vpc_id = aws_vpc.myapp-vpc.id
...
resource "aws_default_security_group" "myapp-default-sg" {  # Configuring the default security group
  vpc_id = aws_vpc.myapp-vpc.id
...
resource "aws_instance" "myapp-server" {
  ...
  provisioner "file" {
    ...
    destiination = "/home/ec2-user/entry-script-on-ec2.sh"
  }
